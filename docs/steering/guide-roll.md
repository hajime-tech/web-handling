...
??? question "演習1: スプレッダ選定"
    塗工直後の光学フィルム（表面接触NG、極めて薄い）のしわ抑制のために選定するスプレッダロールとして最も適切なのは？
...
??? question "演習3: スプレッダロールの種類"
    幅広・高速ライン（>300 m/min）のしわ取りに最適なのはどれか。理由とともに答えよ。
    (a) コンケイブロール  (b) バナナロール（高速対応軸受）  (c) D-bar  (d) テンター

    ??? success "解答"
        **(b) バナナロール（高速対応軸受）**。
        理由：高速・幅広に対する拡幅効果が強力で、装置として安定。コンケイブはマイルド、D-bar は低速向け、テンターは大型で工程組み込みが大変。
