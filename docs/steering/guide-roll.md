...
??? question "演習: 用途の特定"
    「スリット後の複数本ウェブを別々の巻取機に分配する」用途で使うロールはどの分類か。

    ??? success "解答"
        **分離用途**。テーパロールやフリースプレッダで CD 方向に広げて、スリット後のウェブをそれぞれの巻取機方向に分配する。
        橋本『入門』第7章「ローラによるウェブの分離・拡張メカニズム」が詳しい。
...
??? question "演習: 塗工直後の選定"
    塗工直後の光学フィルム（表面接触NG、極めて薄い）のしわ抑制に最適なスプレッダロールは？
    (a) バナナロール  (b) コンケイブロール  (c) エアタンクスプレッダ／非接触式  (d) スパイラル溝付きロール

    ??? success "解答"
        **(c) エアタンクスプレッダ／非接触式**。塗工直後の表面接触が禁忌の場合、接触式は塗膜を傷つける。
        非接触エア式または下流に十分な距離を空けてから接触式を使う構成にする。
...
??? question "演習: 高速ライン向け選定"
    幅広・高速ライン（>300 m/min）のしわ取りに最適なのはどれか。
    (a) コンケイブロール  (b) バナナロール（高速対応軸受）  (c) D-bar  (d) テンター

    ??? success "解答"
        **(b) バナナロール（高速対応軸受）**。
        高速・幅広に対する拡幅効果が強力で、装置として安定。コンケイブはマイルド、D-bar は低速向け、テンターは大型で工程組み込みが大変。
        ただし回転抵抗大の問題があるため、低張力ウェブでは設置位置・巻き付き角を工夫する。
...
??? question "演習: EPC形式の選定"
    巻出機を含むフレーム全体をスライドさせて、原反のオフセットを補正したい場合、最も適したEPC形式は？

    ??? success "解答"
        **エンドポジション型（End-Position Guide）**。
        巻出・巻取直結の用途で、フレーム全体をリニアアクチュエータで動かす。原反の幅方向初期位置のずれを大きく補正できる。
        中間スパンならピボットやパラレル型を使う。
...
??? question "演習: 巻き付き角の不足"
    バナナロールを設置したが拡幅効果が出ない。考えられる原因を 2 つ挙げよ。

    ??? success "解答"
        (1) **取付方向が逆**：弓の凸側がウェブ進入方向を向いている必要がある。
        (2) **巻き付き角不足**：90° 以上を確保しないと効果が出ない。
        その他の原因：曲率（湾曲量）が不足、ロール周面の摩擦不足で滑っている、など。
...
??? question "演習: 設置位置の優先"
    新規ラインで EPC を1台だけ追加できる予算しかない。どこに設置するのが最も効果的か。

    ??? success "解答"
        **巻取直前（エキジットガイド）**。
        巻取での蛇行はテレスコープに直結し、製品全数廃棄リスクが大きい。
        2台目を追加できる場合は巻出直後（エントリーガイド）で原反由来の蛇行を最初に吸収するのが理想。
...
??? question "演習: スパイラル溝の保守"
    スパイラル溝付きロールの拡幅効果が落ちた。最も疑うべき原因は？

    ??? success "解答"
        **溝の目詰まり**。塵・塗工剤・湿気が溝に詰まると、軸方向力が発生しなくなり、平滑ロール化する。
        対策：エアブロー清掃、溶剤拭き取り（材質と相性確認）、目詰まり前の予防保全。
...
??? question "演習: チェックリストの優先項目"
    ガイドロール選定でチェックリスト上の項目のうち、最も忘れがちで失敗例が多いのは？

    ??? success "解答"
        **「巻き付き角 90° 以上の確保」**。
        スペースの都合で巻き付き角を浅くしたガイドロールは、傾けてもウェブを動かす効果が極めて弱く、EPC が機能しない。
        既存装置への後付けで特に失敗しやすいポイント。設置前にレイアウト図で必ず巻き付き角を確認する。
