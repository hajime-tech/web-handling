...
??? question "演習: 蛇行原因の頻度"
    実機で観察される蛇行の原因として、最も多いのは次のうちどれか。
    (a) ウェブのバギング  (b) ロールミスアライメント  (c) 張力分布の片寄り  (d) 環境変動

    ??? success "解答"
        **(b) ロールミスアライメント**。
        装置経年変化、フレーム変形、軸受摩耗、固定金具ガタなど原因は多岐にわたる。
        定期的な平行度測定（レーザアライナで 0.05 mm/m 以下）が対策の基本。
...
??? question "演習: 横方向作用の分類"
    乾燥炉で片側だけ温度が高く、その側のウェブが大きく収縮するために蛇行が発生している。これは橋本分類のどれに該当するか。

    ??? success "解答"
        **(a) 横方向力**。
        温度・湿度差による収縮差は CD 方向に直接働く力としてウェブを動かす。
        対策は乾燥炉の温度プロファイル均一化、または下流に EPC を設置して補正。
...
??? question "演習: 高速化と蛇行"
    ライン速度を 1.5 倍にアップしたところ、それまで安定していたラインが急に蛇行が大きくなった。考えられる物理メカニズムは？

    ??? success "解答"
        **空気同伴による直角方向進入性の崩壊**。
        速度上昇 → 空気膜厚さ増 → 実効摩擦係数低下 → ウェブがロール表面で滑り、ロール軸方向に縛り付けられなくなる → 蛇行拡大。
        対策：マイクログルーブローラ、真空ロール、ニップ追加。
...
??? question "演習: 周波数からの原因推定"
    蛇行波形を FFT すると、巻出ロール回転周期に同期した成分が大きい。考えられる原因は？

    ??? success "解答"
        **巻出原反のバギング、または巻出機の軸偏心／軸受不良**。
        巻出周期と同期する蛇行は、原反由来か巻出機機械要因のいずれか。
        前者なら原反受入検査強化、後者なら軸受交換・芯出し再調整。
...
??? question "演習: センサ選定"
    透明 PET フィルムの蛇行検出に最も適したセンサは？

    ??? success "解答"
        **超音波エッジセンサ**。
        透過光式は透明だと光が通り抜けて検出できない。超音波は透明・半透明・不透明問わず動作する。
        高精度（±0.05 mm）が必要ならラインスキャンカメラ＋画像処理も候補。
...
??? question "演習: 対策の優先順位"
    新規ラインの蛇行対策で、最も基本的（最初に取り組むべき）なものは次のうちどれか。
    (a) EPC 装置を追加  (b) スプレッダロール追加  (c) ロール平行度・真直度を測定／調整  (d) PID チューニング見直し

    ??? success "解答"
        **(c) ロール平行度・真直度を測定／調整**。
        蛇行原因の大半はロールミスアライメント。これを放置したまま EPC を追加しても根本解決にならず、EPC の負荷が増えるだけ。
        まず装置精度を確保してから、必要に応じて EPC・スプレッダ等を検討するのが正しい順序。
