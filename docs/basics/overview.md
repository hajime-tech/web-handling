...
??? question "演習: ウェブの定義"
    次のうち、本サイトでいう「ウェブ」に該当しないものはどれか。
    (a) 厚さ 12 μm の PETフィルム
    (b) 厚さ 100 μm のアルミ箔
    (c) 厚さ 5 mm の鉄板（幅 1m、長さ 100 m）
    (d) 坪量 80 g/m² の上質紙

    ??? success "解答"
        **(c)**。厚さ 5 mm の鉄板は厚すぎ、面外剛性が高く、「面内剛性は高いが面外剛性ほぼゼロ」というウェブの特殊性に当てはまらない。ウェブは厚さ数 μm〜数百 μm が一般的。
...
??? question "演習: 4つの基本課題"
    本サイトで扱う「ウェブハンドリングの4つの基本課題」を答えよ。

    ??? success "解答"
        (1) **張力制御**、(2) **ロール・搬送系の力学**、(3) **蛇行制御**、(4) **巻取り**。
        これら4領域は相互に強く結合し、独立に解くことはできない。
...
??? question "演習: 曲げ剛性の比"
    ウェブの厚さを 50 μm から 25 μm に半減させると、曲げ剛性 $D = E h^3 / 12(1-\nu^2)$ は何倍になるか。

    ??? success "解答"
        厚さ $h$ の **3乗** で効くので、$(1/2)^3 = 1/8$ 倍。
        薄物化は曲げ剛性を急減させ、しわ・座屈が極めて生じやすくなる。
...
??? question "演習: 座標系の対応"
    ...
    ??? success "解答"
        (a) **MD（Machine Direction）**：流れ方向（長手）
        (b) **TD / CD（Transverse / Cross Direction）**：幅方向
        (c) **ND（Normal Direction）**：厚さ方向
