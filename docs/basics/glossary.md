...
??? question "演習1: 用語の対応"
    次の英語用語に対応する日本語は何か。
    (a) telescoping  (b) blocking  (c) normal entry rule  (d) wound-on tension

    ??? success "解答"
        (a) **テレスコープ**（巻取ロールが軸方向にずれ落ちる不良）
        (b) **ブロッキング**（巻取ロール内でウェブ同士が貼り付く現象）
        (c) **ウェブの直角方向進入性**（ウェブが下流ロール軸に直角に進入する原理）
        (d) **WOT/巻取応力**（実際に巻取ロールに残る応力）

??? question "演習2: 記号の意味"
    次の記号は何を表すか答えよ。
    (a) $\sigma_{0.2}$  (b) $\mu_{\text{eff}}$  (c) $\theta_w$  (d) $t = T/w$

    ??? success "解答"
        (a) **耐力**（0.2% proof stress、永久ひずみ 0.2% に対応する応力。非鉄金属で降伏応力の代わりに用いる）
        (b) **有効摩擦係数**（空気膜等の影響を含めた実効摩擦係数）
        (c) **巻き付き角**（ウェブがロールに巻き付く中心角、rad）
        (d) **単位幅張力**（線張力、N/m）

??? question "演習3: 変形モードの分類"
    紙が湿度上昇で幅方向に伸びる現象は、橋本『基礎理論と応用』第2章で分類される基本変形モード（引張・圧縮・せん断・曲げ・ねじり・座屈）のうち、何に近いか。

    ??? success "解答"
        厳密にはどれにも該当せず、**湿度膨張（hygro-expansion）** という別現象だが、力学的には材料自体が「伸ばされた」のと等価で、見かけは引張（伸び）変形と同じ。CD方向に拘束されると、内部に引張応力ではなく **圧縮応力** が発生してしわ（座屈）の原因となる。
