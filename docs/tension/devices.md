...
??? question "演習: 選定の順序"
    張力制御システムを設計するとき、最初に決めるべきは「センサ」「アクチュエータ」「コントローラ」のどれか。

    ??? success "解答"
        いずれでもなく **「目標性能（最大張力／最小張力／応答性／精度）」**。
        目標性能が決まらないと、必要なセンサ容量・アクチュエータ性能・コントローラ仕様も決まらない。
        現場では「何 N/m を ±何 % で制御したい」「加減速 ±何秒で立ち上げたい」を明確にすることから始める。
...
??? question "演習: テンションセンサ容量選定"
    最大張力 500 N を見込むラインのテンションセンサ容量は何 N を選ぶべきか。理由とともに答えよ。

    ??? success "解答"
        **750〜1500 N**（想定最大の 1.5〜3 倍）。
        加減速時の過渡応答や、外乱・突発張力ピークがあるため、最大張力ぎりぎりだと過負荷で破損する恐れがある。
        過負荷耐性は 150% 以上を要求するのが標準。
...
??? question "演習: アクチュエータ選定"
    巻取径が大きく変化する高張力フィルムラインで、巻出側の張力制御に最も適したアクチュエータは？
    (a) パウダブレーキ  (b) ヒステリシスブレーキ  (c) AC サーボモータ  (d) 空圧シリンダ

    ??? success "解答"
        **(c) AC サーボモータ**。
        トルク・速度・位置の高精度制御、巻径変化に応じた指令計算、加減速時の慣性補償が可能。
        パウダブレーキは安価だが応答遅・発熱大で大型ラインには不向き。
...
??? question "演習: コントローラの選定"
    1ライン単機で、ロードセル入力＋アナログ出力で完結する小規模ラインに最も適したコントローラ構成は？

    ??? success "解答"
        **(a) 専用テンションコントローラ**。
        短期立上げが可能で、テーパテンション・巻径演算・PID自動チューニング等の機能内蔵。コスト・工数とも最小。
        ライン全体統括や多軸同期が要件なら PLC+サーボに格上げ。
...
??? question "演習: チューニング手順"
    新規ラインで PID チューニングを始めるとき、最初に動かすゲインは「P」「I」「D」のどれが安全か。

    ??? success "解答"
        **積分（I）のみ（または弱い比例 P）**。
        最初から大きな P や D を入れると発振するリスクが高い。
        まず I だけで応答を確認し、その後 P を徐々に上げて発振直前で 50% 戻す。
        D は通常不要、ノイズに弱い。
...
??? question "演習: 慣性補償トルク"
    巻取機の慣性 $J = 50\,kg \cdot m^2$、巻取径 $R = 0.3\,m$、ライン加速度 $\dot V = 0.5\,m/s^2$ のとき、加速時の張力誤差を最小化するために必要な **慣性補償トルク** はいくらか。

    ??? success "解答"
        角加速度 $\dot\omega = \dot V / R = 0.5/0.3 = 1.67\,rad/s^2$
        慣性トルク $\tau_J = J \dot\omega = 50 \times 1.67 = 83.3\,N\cdot m$
        この大きさを **フィードフォワード**で加算しないと、加速時に張力が急変する。
...
??? question "演習: ターンダウン比"
    張力指令の上限 500 N/m、下限 50 N/m のラインのターンダウン比は？
    また、これがアクチュエータ選定にどう影響するか。

    ??? success "解答"
        ターンダウン比 = 500/50 = **10:1**。
        ターンダウン比が大きいほどアクチュエータの分解能・制御性が要求される。
        パウダブレーキは励磁電流の制御範囲が限られるので 5:1 程度が目安、AC サーボなら 100:1 以上が容易。
        10:1 なら AC サーボが安全な選択。
