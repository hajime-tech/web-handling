...
??? question "演習: ニップ用途の選定"
    上下流のテンションゾーンを完全に分離したい場合、ニップロールは次のうちどの用途で使われるか。

    ??? success "解答"
        **テンションカット**。
        オイラー式 $T_2/T_1 \le e^{\mu\theta}$ の制約を受けずに任意の張力差を維持できるため、駆動分離が不可欠な工程（塗工部前後等）に必須。
...
??? question "演習: 加圧源の選定"
    高い再現性で加圧を制御したい精密用途のニップ。加圧源として最も適切なのは？
    (a) 空圧シリンダ  (b) 油圧シリンダ  (c) 電動シリンダ

    ??? success "解答"
        **(c) 電動シリンダ**。
        サーボ駆動で位置・荷重を高精度に制御可能。空圧は安価で応答良いが力のばらつき大、油圧は高荷重向きだが配管・漏れ管理が複雑。
...
??? question "演習: ゴム硬度の選定"
    段差のあるウェブ（厚みムラ大）を均一にニップしたい。Shore A 50 と A 90 のどちらが適切か。

    ??? success "解答"
        **Shore A 50（軟質）**。
        軟質ゴムは追従性が高く、ウェブの段差や厚みムラを吸収して均一なニップ圧を維持できる。
        ただし自己発熱と摩耗が大きいので、速度・荷重には注意。
...
??? question "演習: 巻取とニップの効果"
    巻取張力 $T_w = 200\,N/m$ だけで巻くと軟巻きになる。ライダロールでニップ荷重 $N = 5000\,N/m$、層間摩擦係数 $\mu = 0.3$ を加えると、Pfeiffer 式による巻取応力（単位幅）はどう変化するか。

    ??? success "解答"
        ニップなし：$T_w = 200\,N/m$
        ニップあり：$T_w + \mu N = 200 + 0.3 \times 5000 = 200 + 1500 = 1700\,N/m$
...
??? question "演習: ニップ跡対策"
    ラミネート工程で「ニップ跡（線状の痕）」がウェブに転写されている。最も優先すべき対策は？

    ??? success "解答"
        **ニップ荷重を下げる、または可変クラウン（Swimming Roll, Nipco）を採用する**。
        ニップ跡は過荷重あるいはクラウン不適による局所過大圧が主因。
        まずは荷重を下げて必要最小限の圧力に。次にロール撓みを補正する可変クラウンを検討。
